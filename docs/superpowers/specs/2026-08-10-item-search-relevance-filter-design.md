...
- normal item-name search;
- upgrade/crysta name search;
- CLI and Discord, because both consume the same ranked candidate lists.

Stat searches and upgrade graph contents are outside this change.
...
Candidates below that relevance rule are removed completely rather than retained for later pages.

The threshold is intentionally shared and deterministic. No Qwen call is introduced for candidate filtering.
...
For unrelated input such as `asdfgh`, if no item clears the lexical/fuzzy relevance rule, the result list is empty. The existing frontend no-match/unclear-query handling should be used rather than filling the page with unrelated names.

## Architecture

`rank_items(query, items)` remains the single shared ranking entry point. It will:

1. normalize and reject too-short queries as today;
2. score each supplied candidate with `_score_item()` as today;
3. discard candidates that fail the relevance rule;
4. sort the retained candidates using the existing ordering;
5. return only the retained candidates.
...
- cap results to an arbitrary top-N;
- change page size;
- change exact-name resolution;
- change item-type/stat parsing;
- change Qwen/fallback behavior;
- change upgrade graph discovery or hybrid tree display;
- change stat-result filtering or ranking.

## Tests

Add focused regression coverage proving:

- strong lexical matches remain even if their adjusted score would otherwise be near a fuzzy threshold;
- fuzzy typo matches at or above the relevance threshold remain;
- fuzzy-only candidates below the threshold are removed;
- a normal item-name search does not expose unrelated database items;
- an upgrade-name search does not expose unrelated crystas;
- a completely unrelated name query returns an empty candidate list;
- existing exact item and exact upgrade resolution behavior remains unchanged.

Both direct `rank_items()` tests and `SearchService` integration tests should be used so the shared algorithm and the two affected search flows are covered.
